...
void* ArenaAllocator::allocate(std::size_t size, std::size_t alignment) {
    ...
    // 1. Determine if we even have enough space to allocate this, otherwise throw or return a nullptr
    ...
    if (std::align(alignment, size, aligned_ptr, remaining_space) == nullptr) { // Alignment failed, we didn't have enough space
        // Alternatively:
        // return nullptr;
        throw std::bad_alloc();
    }
